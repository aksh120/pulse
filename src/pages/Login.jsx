...
import { ArrowRight, Eye, EyeOff, Check, AlertCircle, Sun, Moon, Lock, ShieldCheck } from 'lucide-react';
    ...
    <div className="min-h-screen bg-pulse-bg dark:bg-pulse-dark-bg text-pulse-primary dark:text-pulse-dark-primary flex flex-col justify-between transition-colors duration-200">
      ...
      <header className="px-6 py-4 flex items-center justify-between border-b border-pulse-border/70 dark:border-[#252830]">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            to="/signup"
            className="text-xs font-semibold text-pulse-secondary hover:text-pulse-primary dark:text-pulse-dark-secondary dark:hover:text-white px-3 py-1.5 rounded-lg hover:bg-pulse-surface dark:hover:bg-[#161822] transition-colors"
          >
            Create account
          </Link>
        </div>
      ...
      <main className="flex-1 flex items-center justify-center p-4 sm:p-8">
        <div className="w-full max-w-4xl grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          <div className="lg:col-span-7 bg-white dark:bg-[#13151C] p-6 sm:p-10 rounded-3xl border border-pulse-border dark:border-[#232630] shadow-card">
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              <button
                ...
                onClick={() => {
                  setIsLoading(true);
                  setTimeout(() => { setIsLoading(false); setSuccess(true); setTimeout(() => navigate('/'), 1000); }, 600);
                }}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {success && (
                <div className="flex items-center gap-2 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 text-xs border border-emerald-200 dark:border-emerald-900/50">
                  ...
                  <span>Logged in successfully! Redirecting to workspace...</span>
                </div>
              )}
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-xs font-semibold text-pulse-primary dark:text-white">
                    Password
                  </label>
                  <a href="#" className="text-[11px] font-medium text-pulse-accent hover:underline">
                    Forgot password?
                  </a>
                </div>
              ...
              <div className="flex items-center">
                <label className="flex items-center gap-2 cursor-pointer text-xs text-pulse-secondary dark:text-pulse-dark-secondary">
                  <input
                    type="checkbox"
                    checked={rememberMe}
                    onChange={(e) => setRememberMe(e.target.checked)}
                    className="rounded border-pulse-border text-pulse-accent focus:ring-pulse-accent"
                  />
              ...
              >
                {isLoading ? 'Signing in...' : 'Log in to PULSE'}
              </Button>
            </form>

            <div className="mt-6 text-center text-xs text-pulse-secondary">
              Don't have an account?{' '}
              <Link to="/signup" className="text-pulse-accent font-semibold hover:underline">
                Start for free
              </Link>
            </div>
          </div>
          ...
          <div className="hidden lg:flex lg:col-span-5 flex-col justify-between p-8 rounded-3xl bg-gradient-to-br from-[#EEEDFC] via-[#F6F5FF] to-[#ECEBFA] dark:from-[#18162E] dark:via-[#1B1934] dark:to-[#141226] border border-[#E0DEF7] dark:border-[#2E2A55] min-h-[460px]">
            <div>
              <Badge variant="purple" size="sm" className="mb-4">
                ENTERPRISE READY
              </Badge>
              <h2 className="text-2xl font-extrabold text-pulse-primary dark:text-white tracking-tight leading-snug mb-4">
                The operations intelligence layer for modern engineering teams.
              </h2>
              <p className="text-xs text-pulse-secondary dark:text-pulse-dark-secondary leading-relaxed mb-6">
            ...
            <div className="pt-6 border-t border-[#DCD9F5] dark:border-[#2D2A54] space-y-2 text-[11px] text-pulse-secondary dark:text-pulse-dark-secondary font-medium">
              <div className="flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-pulse-accent" />
                <span>SOC2 Type II Certified & GDPR Compliant</span>
              </div>
              <div className="flex items-center gap-2">
                <Lock className="w-4 h-4 text-pulse-accent" />
                <span>End-to-end 256-bit AES database encryption</span>
              </div>
            </div>
          </div>
